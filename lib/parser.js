...
function parseVariableBlock(varName, blockContent) {
  ...
  if (descMatch) {
    field.description = descMatch[1].replace(/\\"/g, '"');
  }
  ...
  if (field.default === undefined && !blockContent.includes('nullable = true')) {
    field.required = true;
  }
  ...
  // Parse validation blocks to see if we restrict values to a select dropdown
  const validationRegex = /validation\s*\{([\s\S]+?)\}/g;
  let validationMatch;
  while ((validationMatch = validationRegex.exec(blockContent)) !== null) {
    const validationBlock = validationMatch[1];
    const conditionMatch = validationBlock.match(/condition\s*=\s*([^\n]+)/);
    if (conditionMatch) {
      const conditionStr = conditionMatch[1].trim();
      // Regex to find contains(["A", "B"], var.name) or contains(["A", "B"], upper(var.name))
      ...
      if (containsMatch) {
        ...
        if (options.length > 0) {
          ...
          break; // Stop after finding the first valid contains pattern
        }
      }
    }
  }
